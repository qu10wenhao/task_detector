...
{
	int* count = bpf_map_lookup_elem(&trace_cpu_maps, &cpu);
	if (count) {
		(*count)++;
	} else {
		int one = 1;
		bpf_map_update_elem(&trace_cpu_maps, &cpu, &one, BPF_NOEXIST);
	}
}

static __always_inline void set_trace_off(int cpu)
{
	int* count = bpf_map_lookup_elem(&trace_cpu_maps, &cpu);
	if (count)
		(*count)--;
...
static __always_inline void handle_trace(void *ctx, struct ti_key *tik, int type)
{
	...
	if (output_log) {
		 ...
		 * SYSCALL "epoll_wait" events which are called by tasksnoop 
